...
export default function ResellerClientes() {
  ...
  async function cargar() {
    setLoading(true);
    try {
      const [c, p, pf] = await Promise.all([
        apiFetch('/reseller/clientes'), apiFetch('/reseller/planes'), apiFetch('/reseller/perfil'),
      ]);
      setClientes(c.clientes);
      setPlanes(p.planes);
      setPerfil(pf.perfil);
      setForm((f) => ({ ...f, plan_id: f.plan_id || p.planes[0]?.id || '' }));
      apiFetch('/reseller/clientes/estados').then((e) => setEstados(e.estados)).catch(() => {});
  ...
  async function accion(c, path) {
    setBusy(c.id);
    try { await apiFetch(`/reseller/clientes/${c.id}/${path}`, { method: 'POST' }); await cargar(); }
    catch (e) { alert(e.message); } finally { setBusy(null); }
  }
  async function borrar(c) {
    if (!confirm(`¿Eliminar "${c.nombre_empresa}" y su estación?`)) return;
    setBusy(c.id);
    try { await apiFetch('/reseller/clientes/' + c.id, { method: 'DELETE' }); await cargar(); }
    catch (e) { alert(e.message); } finally { setBusy(null); }
  }
  ...
  const sinCupo = perfil && perfil.disponibles <= 0;

  return (
    <div className="space-y-6">
      <div className="card p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold">Mis radios <span className="text-gray-400 font-normal">({clientes.length}{perfil ? `/${perfil.cupo_radios}` : ''})</span></h2>
          <div className="flex items-center gap-2">
            <button onClick={cargar} className="btn-ghost !py-2 !px-3 text-xs"><IconRefresh width={15} height={15} /> Actualizar</button>
            <button onClick={() => { setMsg(null); setOpen(true); }} disabled={sinCupo}
              className="btn-primary !py-2 !px-3 text-xs disabled:opacity-50" title={sinCupo ? 'Cupo agotado' : ''}>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="4" className="py-8 text-center text-gray-400">Aún no tienes radios. ¡Crea la primera!</td></tr>
              ) : (
                clientes.map((c) => {
                  const est = BADGE[estados[c.id]] || BADGE.offline;
                    ...
                    <tr key={c.id} className="border-b border-gray-50 dark:border-gray-800/60 last:border-0">
                      ...
                      <td className="py-3 pl-3">
                        <div className="flex items-center justify-end gap-1.5">
                          {busy === c.id ? <span className="text-xs text-gray-400 px-2">…</span> : (
                            <>
                              <IconBtn title="Iniciar" onClick={() => accion(c, 'iniciar')}><IconPlay width={14} height={14} /></IconBtn>
                              <IconBtn title="Parar" onClick={() => accion(c, 'parar')}><IconStop width={13} height={13} /></IconBtn>
                              {suspendido
                                ? <IconBtn title="Reactivar" onClick={() => accion(c, 'reactivar')}><IconPower width={14} height={14} /></IconBtn>
                                : <IconBtn title="Suspender" onClick={() => accion(c, 'suspender')}><IconPower width={14} height={14} /></IconBtn>}
                              <IconBtn title="Entrar al panel" onClick={() => entrar(c)}><IconEnter width={14} height={14} /></IconBtn>
                              <IconBtn title="Eliminar" onClick={() => borrar(c)} red><IconTrash width={14} height={14} /></IconBtn>
                            </>
                          )}
                        </div>
                      </td>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Crear nueva radio">
        <form onSubmit={crear} className="space-y-3">
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button className="btn-primary flex-1" disabled={saving}><IconPlus width={16} height={16} /> {saving ? 'Creando…' : 'Crear radio'}</button>
          </div>
        </form>
      </Modal>
    </div>
...
function IconBtn({ children, title, onClick, red }) {
  return (
