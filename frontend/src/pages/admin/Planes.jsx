...
export default function AdminPlanes() {
  ...
  function abrirEditar(p) {
    setEditId(p.id);
    setForm({
      nombre: p.nombre, max_bitrate: p.max_bitrate, max_oyentes: p.max_oyentes,
  ...
  async function guardar(e) {
    ...
    try {
      if (editId) await apiFetch('/admin/planes/' + editId, { method: 'PUT', body: JSON.stringify(form) });
      else await apiFetch('/admin/planes', { method: 'POST', body: JSON.stringify(form) });
      setOpen(false);
      cargar();
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={abrirCrear} className="btn-primary !py-2 !px-3 text-xs">
          <IconPlus width={15} height={15} /> Crear plan
        </button>
      </div>
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  <button onClick={() => abrirEditar(p)}
                    className="text-xs px-2.5 py-1 rounded-lg border border-gray-200 dark:border-gray-800 hover:border-brand-500 hover:text-brand-600 transition">Editar</button>
